...
gene2mod = gene2mod[  is.element( gene2mod[,2],selectedModuleID )  , ]
...
for( i in 1:nrow(hub_inter)  ){
 # print(i)
  ...
  temp_hub_ppi = temp_hub_ppi[ order( temp_hub_ppi[,3], decreasing = T )  , ]
...
#    temp_mod_score = GSEA.EnrichmentScore( temp_hub_ppi[,2], temp_mod_gene, 1, temp_hub_ppi[,3] )
    temp_mod_score = GSEA.EnrichmentScore( temp_hub_ppi[,2], temp_mod_gene, 0, NULL )
...
#    rand_cor = abs( cor( as.numeric(rand_exp_hub) , t(as.matrix(rand_exp_nb) ) )[1,] )
    ...
    rand_hub_ppi = cbind( temp_hub_ppi[,1:2], corR=sample(temp_hub_ppi[,3]))
    rand_hub_ppi = rand_hub_ppi[ order(rand_hub_ppi[,3],decreasing = TRUE)  ,]
...
#      temp_mod_score = GSEA.EnrichmentScore( rand_hub_ppi[,2], temp_mod_gene, 1, rand_hub_ppi[,3] )
      temp_mod_score = GSEA.EnrichmentScore( rand_hub_ppi[,2], temp_mod_gene, 0, NULL )
      temp_ESscore = c(temp_ESscore, temp_mod_score$ES)
    }
